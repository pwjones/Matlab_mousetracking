function data =  noseTrajSniff_1Mouse(mouse_name, dirs, varargin)
...
if nargin > 2
    ...
    %first load the all of the experiment files
    %data = struct('vids', {}, 'camTrig', {}, 'resp', {});
    for ii = 1:length(dirs)
        load([VIDEO_ROOT filesep dirs{ii} filesep dirs{ii} '_' mouse_name '.mat'], 'exp');
        if ii == 1
            data = exp;
        else
            data.vids = cat(2,data.vids, exp.vids);
            data.resp = cat(2, data.resp, exp.resp);
            data.camTrig = cat(2, data.camTrig, exp.camTrig);
        end
    end
end
